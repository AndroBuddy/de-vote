...
<template>
  ...
  <div class="flex flex-col gap-6 transition-transform" v-else>
    <div class="flex whitespace-nowrap flex-nowrap lg:flex-wrap overflow-auto gap-8 relative -ml-20 lg:ml-0">
      <section
        class="sticky top-0 left-0 flex items-center h-full lg:h-0 lg:hidden p-6 bg-gradient-to-r from-[#f5f5f5] z-10 transition-transform duration-[350ms] ease-[cubic-bezier(0.37,0,0.63,1)]"
        :class="[
      ...
      >
        <router-link
          :to="`/p/${item._id}`"
          class="hover:-translate-y-2 hover:scale-105 transition-transform"
        >
          <div
      ...
      <section
        class="sticky top-0 right-0 h-full lg:h-0 lg:hidden p-6 bg-gradient-to-l from-[#f5f5f5] md:flex"
      ></section>
    </div>
  </div>
</template>
